...
const Profile = () => {
    ...
    useEffect(() => {
        if (user) {
            ...
            setProfile({
                firstName: fName || user.email?.split('@')[0] || '',
                lastName: lName || '',
                email: user.email || '',
    ...
    const handleUpdateProfile = async (e) => {
        ...
        try {
            ...
            // The backend returns the updated user object with "name"
            // We need to ensure our local context reflects the split for the UI
            // But since we just sent firstName and lastName, we can use those to update local state
            // or rely on the backend response if it returns the combined name.

            // Let's assume backend returns "name".
            // We can manually reconstruct "name" for the context if needed, 
            // but effectively we want to update the context with the new data.
