...
import { Shield, Globe, Lock, Cpu, Search, Terminal, ArrowLeft, BookOpen, Layers, Zap, Loader2 } from 'lucide-react';
...
const getCategoryIcon = (category) => {
    switch (category?.toLowerCase()) {
        case 'network': return <Globe size={32} />;
        case 'offensive': return <Zap size={32} />;
        case 'investigation': return <Search size={32} />;
        case 'defense': return <Terminal size={32} />;
        case 'cloud': return <Cpu size={32} />;
...
const getCategoryColor = (category) => {
    switch (category?.toLowerCase()) {
        case 'network': return "from-cyan-500/10 to-transparent";
        case 'offensive': return "from-red-500/10 to-transparent";
        case 'investigation': return "from-purple-500/10 to-transparent";
        case 'defense': return "from-emerald-500/10 to-transparent";
        case 'cloud': return "from-blue-600/10 to-transparent";
        ...
        <div className="bg-[#050505] min-h-screen text-white font-plus-jakarta overflow-x-hidden selection:bg-lh-purple selection:text-white">
                ...
                <div className="max-w-[1300px] mx-auto">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-32 space-y-6">
                            ...
                            <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.6em]">Initializing_Registry...</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 auto-rows-[280px]">
                            {courses.map((course, idx) => {
                                // Determine grid size: index 1 and 4 are double-width (for visual variety)
                                const isDouble = idx === 1 || idx === 4 || (idx > 5 && (idx - 1) % 5 === 0);
                                const gridSize = isDouble ? "md:col-span-2" : "col-span-1";
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            <div className={`absolute inset-0 bg-gradient-to-br ${getCategoryColor(course.category)} opacity-0 group-hover:opacity-100 transition-opacity duration-700`}></div>
                                            ...
                                            <div className="relative z-10 flex flex-col h-full">
                                                <div className="flex justify-between items-start mb-6">
                                                    ...
                                                    <div className="flex flex-col items-end gap-2">
                                                        <span className="text-[9px] font-black uppercase tracking-widest px-2 py-1 bg-white/5 rounded-md text-white/40 group-hover:text-lh-purple group-hover:bg-lh-purple/10 transition-all">
                                                            {course.level}
                                                        </span>
                                                        <span className="text-[9px] font-black uppercase tracking-widest px-2 py-1 bg-lh-purple/10 rounded-md text-lh-purple">
                                                            {course.category}
                                                        </span>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-3 pt-6 border-t border-white/5 mt-auto">
                                                    <div className="flex-grow flex items-center gap-2">
                                                        ...
                                                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">
                                                            {course.chapters?.length || 'Dynamic'} Chapters
                                                        </span>
                                                    </div>
            ...
            <section className="py-24 px-6 relative z-10 bg-white/[0.01] border-t border-white/5">
                <div className="max-w-[1300px] mx-auto text-center space-y-10">
                    ...
                    <button
                        onClick={() => toast.info("Brochure request feature coming soon!")}
                        className="px-16 py-6 bg-lh-purple text-white rounded-full font-black text-xs uppercase tracking-[0.4em] shadow-[0_0_50px_rgba(188,19,254,0.3)] hover:bg-white hover:text-black transition-all transform hover:scale-105 duration-500"
                    >
