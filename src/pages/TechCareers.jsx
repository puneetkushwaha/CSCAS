...
const TechCareers = () => {
    ...
    const getBentoProps = (career, idx) => {
        const colors = [
            ...
            "from-lh-purple/10 to-transparent",
    ...
    const totalPages = Math.ceil(techCareers.length / ITEMS_PER_PAGE);
    const currentCareers = techCareers.slice(currentPage * ITEMS_PER_PAGE, (currentPage + 1) * ITEMS_PER_PAGE);
    ...
    return (
        <div className="bg-[#050505] min-h-screen text-white font-plus-jakarta overflow-x-hidden selection:bg-lh-purple selection:text-white">
                ...
                <div className="max-w-[1300px] mx-auto grid lg:grid-cols-2 gap-16 items-center">
                    ...
                    >
                        <div className="flex items-center gap-3 text-lh-purple">
                            ...
                            <span className="uppercase tracking-[0.4em] text-[11px] font-black">Technical Division v1.0</span>
                        </div>
                        ...
                        <p className="text-gray-400 text-lg font-medium max-w-xl leading-relaxed">
                            Cybersecurity isn't just one job—it's a vast technical landscape. Choose your specialization and build a career that matches your passion.
                        </p>
                        ...
                        <div className="flex flex-wrap gap-4 pt-4">
                            <div className="flex items-center gap-6 px-8 py-4 bg-white/[0.03] border border-white/10 rounded-2xl backdrop-blur-xl">
                                ...
                                <p className="text-[11px] font-black uppercase tracking-widest text-white/60 leading-tight">
                                    ...
                                    <span className="text-lh-purple">Technical Roadmaps</span>
                                </p>
                            </div>
                        </div>
                    </motion.div>
            ...
            {/* --- Section 2: Career Grid --- */}
            <section className="py-24 px-6 relative z-10">
                <div className="max-w-[1300px] mx-auto">
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-20">
                        <div className="flex items-center gap-4">
                            <Scan className="text-lh-purple" size={32} />
                            <h2 className="text-3xl md:text-5xl font-[900] tracking-tighter uppercase">Explore <span className="text-white/20">Specializations</span></h2>
                        </div>
                        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-lh-purple animate-pulse">Showing careers {currentPage * ITEMS_PER_PAGE + 1} - {Math.min((currentPage + 1) * ITEMS_PER_PAGE, techCareers.length)} of {techCareers.length}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 auto-rows-[320px]">
                        {currentCareers.map((career, idx) => (
                            ...
                            >
                                ...
                                >
                                    <div className={`absolute inset-0 bg-gradient-to-br ${career.color} opacity-0 group-hover:opacity-100 transition-opacity duration-700`}></div>
                                    ...
                                    <div className="relative z-10 flex flex-col h-full">
                                        ...
                                        <div className="flex items-center gap-3 pt-6 border-t border-white/5 mt-auto">
                                            ...
                                            <span className="text-[9px] font-black uppercase tracking-[0.3em] text-white/30">Roadmap Available</span>
                                        </div>
                                    </div>
